      ...
      <b-col lg="8">
        <b-card align="center" class="identification-box" title="Connectez-vous">
          <b-col offset-lg="2" lg="8">
            ...
            <b-form >
              <b-form-input class="mt-4 form-control" id="input-1" v-model="form.email" type="email" required placeholder="Entrez votre adresse email"></b-form-input>
...
export default {
  ...
  methods: {
      login() {
        ...
        this.$http.post('http://localhost:3000/api/users/login', user)
          .then(res => {
            if (res.status === 200) {
